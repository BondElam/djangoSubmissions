...
$(document)
		.ready(
				function() {
					...
					$(".btn-edit")
							.click(
									function() {
										...
										editPop.find('input, button, td').each(
												function() {
													this.id = 'ed' + this.id;
												})
					...
					$("#table-submissions").on(
							"click",
							"tbody tr",
							function() {

								$("#table-submissions tr").css(
					...
					$("#table-submissions").on(
							...
							function() {
								...
								case 1:
									$("#srch-story").val($(this).text());
									break;
								case 2:
									$("#srch-words").val($(this).text());
									break;
								case 3:
									$("#srch-file").val($(this).text());
									break;
								case 4:
									var pub = $(this).text();
									$("#srch-publisher option").each(
											function() {
												if ($(this).text() === pub) {
													$(this).prop('selected',
															true);
									...
									break;
								case 5:
									$("#srch-submission-date").val(
											$(this).text());
									break;
								case 6:
									var disp = $(this).text();
									$("#srch-disposition option").each(
											function() {
												if ($(this).text() === disp) {
													$(this).prop('selected',
															true);
					...
					$(document).on(
							'click',
							'#table-dispositions tbody tr',
							function() {

								tbl.lastDispIndex = $(this).index();

								$("#table-dispositions tr").css(
										'background-color', '#e0e0e0');
								$("#table-dispositions tr").css('color',
										'black');
								$(this).css("background-color", "#449d44");
								$(this).css("color", "white");
							})
					...
					$(document)
							.on(
									'click',
									'#table-publishers tbody tr',
									function() {

										tbl.lastPubIndex = $(this).index();

										$("#table-publishers tr").css(
												'background-color', '#e0e0e0');
										$("#table-publishers tr").css('color',
												'black');
...
function editSubmission() {
	...
	if (tbl.sqlDict['action'] === 'Delete') {
		...
		var s = "Click 'OK' to confirm that you want to delete submission #"
				+ id + ".";
		myPop('deleteSub', 'Confirm Deletion', s, 'color:red;',
				"approveSqlDict()", 2);
		return;
	}
	if ($("#edin-btn-save").text() !== 'Add') {
		tbl.sqlDict['submission_id'] = id; // if update vs insert, rest ok for
											// both
	}
	...
	tbl.sqlDict["disposition_id"] = $("#edin-disposition option:selected")
			.val();
...
function approveSqlDict() {
	...
	myPop('updateSubmission', 'Is this sqlDict Ok?', jString, 'color:gray;',
			'sendEdit();', 2);
}
	...
	$.ajax({
		...
		success : function(result) {
			if (tbl.sqlDict['action'] === "Delete") {
				...
				tbl.rowIndex = $('#table-submissions tr').length - 1; // highlight
																		// new
																		// row
...
function repositionTable() {
	if (tbl.lastTop < 0) {
		$("#table-submissions tbody").scrollTop(
				$("#table-submissions tr:last").position().top);
		tbl.lastTop = $("#table-submissions tbody").scrollTop();
	} else {
	...
	if (tbl.resetHighlight) {
		if (tbl.rowIndex >= 0) {
			var row = tbl.rowIndex + 1; // index is 0-based, rowIndex =
										// 1-based??????
			$("#table-submissions tr:nth-child(" + row + ")").css(
					"background-color", "#449d44");
			$("#table-submissions tr:nth-child(" + row + ")").css("color",
					"white");
		}
...
function editDisposition() {
	...
	$
			.ajax({
				url : "../dispositions_table/",
				type : "POST",
				// data: $.param(sqlDict),
				success : function(result) {
					myPop('dispositions_table', "Select Disposition", result,
							'', '', 0);
					tbl.lastDispIndex = -1;
				},
				error : function(result) {
...
function availablePublishers() {
	...
	$
			.ajax({
				url : "../available_publishers/",
				type : "POST",
				data : $.param(dict),
				success : function(result) {
					myPop('publishers_table', "Available Publisher", result,
							'', '', 0);
					tbl.lastPubIndex = -1;
				},
				error : function(result) {
...
function updateDisp(action) {
	...
	s += "<button style='margin-right:10px;' onclick='sendDisposition(&apos;"
...
function myPop(myId, myTitle, myContent, myStyle, myFunc, numberButtons) {
	...
	;
	// var s = '<div id="' + myId + '" class="popup draggable" style="' +
	// myStyle + '">';
	var s = "<span id = '" + myId
			+ "' class='popup draggable' style= 'border: 1px solid #505050;"
			+ myStyle + "'>";
	s += '<h3>' + myTitle + '</h3>';
	s += '<p>' + myContent + '</p>';
	// s += '<br/>';
	if (numberButtons > 0) {
		s += '<button  onclick="' + myFunc
				+ ';$(this).parent().remove()">OK</button>';
	}
	if (numberButtons > 1) {
